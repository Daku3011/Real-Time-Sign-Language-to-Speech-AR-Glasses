    ...
    <div className="w-full h-full overflow-y-auto slide-bg-mesh slide-content-wrapper">
        <div style={{ padding: 'var(--slide-py) var(--slide-px)', maxWidth: '1200px', margin: '0 auto', paddingBottom: '100px' }}>
            ...
            <div className="mb-2 animate-fade-up delay-1">
                <span style={{ color: 'var(--teal)', textShadow: 'var(--shadow-glow)', fontSize: '16px', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase' }}>Slide 6 of 9</span>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '16px', marginBottom: '24px' }}>
                {[
                    {
                        phase: 'Phase 1', timeline: 'Month 1-6', channel: 'B2G — Government (ADIP)', color: 'var(--teal)', textShadow: '0 0 1px rgba(0,180,216,0.3)',
                        steps: [
                    ...
                    },
                    {
                        ...
                        steps: [
                            ...
                            'Target urban professionals & students (highest intent)',
                    ...
                    },
                    {
                        ...
                        steps: [
                            ...
                            'Hospital compliance packages (disability mandates)',
                    ...
                    <div key={i} className={`animate-fade-up delay-${i + 2}`} style={{
                        ...
                        <div className="flex items-center justify-between mb-2">
                            ...
                            <span style={{ fontSize: '15px', color: 'var(--slate)', fontWeight: 500, background: 'var(--light-gray)', padding: '2px 8px', borderRadius: '4px' }}>{p.timeline}</span>
                        </div>
                        <h3 style={{ fontWeight: 700, fontSize: '20px', color: 'var(--charcoal)', marginBottom: '14px' }}>{p.channel}</h3>
                        {p.steps.map((s, j) => (
                            <div key={j} className="flex items-start gap-2 mb-2.5">
                                ...
                                <span style={{ fontSize: '16px', color: 'var(--slate-dark)', lineHeight: 1.4 }}>{s}</span>
                            </div>
                        ))}
                        <div style={{
                            marginTop: '14px', padding: '8px 12px', borderRadius: '6px',
                            background: `${p.color}10`, border: `1px solid ${p.color}25`,
                            fontSize: '16px', fontWeight: 600, color: p.color, textAlign: 'center',
                        }}>
                            Target: {p.target}
                        </div>
                    </div>
            ...
            <div className="animate-fade-up delay-6" style={{
                ...
                <div>
                    <div style={{ fontSize: '15px', color: 'rgba(255,255,255,0.4)', fontWeight: 600, letterSpacing: '0.05em', marginBottom: '12px', textTransform: 'uppercase' }}>
                        Customer Discovery
                    </div>
                    {[
                        ...
                        <div key={i} className="flex items-center gap-2 mb-2">
                            ...
                            <span style={{ fontSize: '16px', color: 'rgba(255,255,255,0.7)' }}>{c}</span>
                        </div>
                    ))}
                </div>
                <div>
                    ...
                    <div style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '31px', color: 'var(--gold)', marginBottom: '6px' }}>100%</div>
                    <p style={{ fontSize: '16px', color: 'rgba(255,255,255,0.6)', lineHeight: 1.5 }}>
                        Our ₹15,000 device is within ADIP's ₹15K full subsidy ceiling. For income ≤₹22,500/month → 100% free. ₹22.5K–₹30K → 50% subsidy. Requires 40%+ disability + UDID card. Gujarat utilizes 75-80% of ADIP funds. Scheme approved till March 2026 (DEPwD).
                    </p>
                </div>
            </div>
        </div>
